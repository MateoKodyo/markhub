...
### PLAN-AI-READY v2 — gelé après clôture v1 (2026-05-21)

PLAN-AI-READY v1 closed (STEPS 1-7 livrés). Candidats v2 si l'usage le réclame :

* **Détection des patterns AI cachés** — `.cursor/rules` / `.cursor/rules/*`, `.github/copilot-instructions.md`, `.aider.conf.yml/.yaml`. Le détecteur (`detector.ts`) les gère déjà et est testé, mais le `vault_scan` Rust ignore toute entrée commençant par `.` (volontaire — garde `.git`, `.svelte-kit` hors sidebar). Les révéler implique une décision UX : montrer `.cursor/` et `.github/` dans l'arbre. Mini-chantier dédié.

* **Highlight de recherche en mode source** — le mode source ne fait qu'une sélection textarea simple (un seul match visible). Multi-highlight dans un `<textarea>` = trick "div miroir", ou bascule du mode source vers CodeMirror 6 (`@codemirror/search` intégré). Différé.

* **Override per-fichier** — `markus-ai-aware: false` en frontmatter pour dé-marquer un fichier détecté automatiquement. v1 n'a pas d'override.

* **Recherche cross-node en preview** — `findRangesInElement` ne matche que dans un même nœud texte ; un terme coupé par du formatage inline (`**gras**`) est manqué.
