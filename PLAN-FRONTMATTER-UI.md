...
> **Single objective**: deliver a polished, dedicated UI block for YAML frontmatter at the top of every markdown file, with a clean read mode and a dedicated edit mode. Read at the start of every Claude Code session working on this plan. **Prerequisite**: PLAN-DESIGN-DEFAULTS must be ✅ complete (this plan consumes the design tokens). **Reference**: `DESIGN-PRINCIPLES.md` governs the visual treatment. **Scope reminder**: this plan is **only about the UI of the frontmatter block**. Parsing uses `js-yaml`. No vault linting, no LLM, no auto-cleaning of malformed YAML.
