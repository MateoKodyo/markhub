...
**1. Pièce à conviction #1 — Crepe est intégralement mocké dans les tests existants.** Dans `tests/component/Editor.test.svelte.ts:6-26`, un `vi.mock('@milkdown/crepe', ...)` remplace `Crepe` par une coquille vide qui ne rend RIEN. Le commentaire l'admet : *« Mock Milkdown so jsdom doesn't choke on contenteditable / ProseMirror. Real Milkdown is exercised in Phase 5 E2E. »* Or `STATE.md` confirme que la Phase 5 E2E real-binary n'a jamais été montée. Donc les bugs visuels Crepe (slash menu, toolbar flottante, headings, frontmatter rendu) ne sont couverts par AUCUN test, ni unit, ni E2E.

**2. Pièce à conviction #2 — résultats expérimentaux du probe diagnostique.** Test exécuté : `npx vitest run tests/component/_diagnostic-crepe-probe.test.svelte.ts`. Résultats :
...
* PanelRight (outline, état actif via background pill-bg) Tooltips au hover, `aria-label`s, ⌘\ hint dans le title.
...
> Mission : exécuter PLAN-THEMING de bout en bout — infra v2, Solar, Tokyo, picker à 2 slots, polish transitions/anti-flash, audit.

## Branche

`feat/theming` — 6 commits, prête pour merge `main` après smoke test Matheo.
...
# Session — 2026-05-14 (soir) — PLAN-EXPORT-MD

> Mission ad-hoc demandée par Matheo : « ajouter un Export file pour exporter le MD en …MD bien propre ». Petit chantier ciblé, hors plan formel, livré + mergé dans la session.

## Cadrage avant TDD

Aiguillage en début de session :

1. **Sémantique** → "Export normalisé" (pipeline propre), pas un simple Save As.
2. **Destination** → file picker système.
3. **Trigger** (multi-select) → command palette + bouton status bar + sidebar context menu.
4. **Conflits noms** → confiance au dialog macOS (pas de double-check Rust).
5. **Strictesse body norm** → minimal (trim trailing whitespace, collapse blank lines, final LF) — pas de normalisation des tokens markdown (listes, emphases, etc.).

Plan annoncé en 4 STEPS (STEP 1 = pipeline pur, STEP 2 = commande Tauri, STEP 3 = API front + dialog, STEP 4 = 3 entrées UI), gates entre chaque.

## Livraison

### STEP 1 — pipeline pur Rust (`commands::export::normalize_markdown_export`)

Module `src-tauri/src/commands/export.rs`.

Comportement :
- Frontmatter YAML (`---\n…\n---\n` en tête) → **passthrough verbatim**. Décision : pas de re-sérialisation YAML (pas de crate yaml dans le projet, et user a demandé "minimal"). Si un jour besoin d'un YAML canonicalisé, c'est un chantier à part.
- Body :
  - CRLF / CR isolé → LF
  - Trim trailing whitespace (espaces + tabs) par ligne
  - Collapse 3+ newlines consécutifs → exactement 2
  - Final newline LF garanti (exactement 1)
- **Blank line forcée** entre frontmatter et body (convention markdown standard).
- Idempotente.

**TDD pas 100% strict** — j'ai écrit impl + tests dans le même flush. 2 tests sur 23 ont fail à la première run et capturé des bugs réels (frontière frontmatter↔body et règle de la blank line) — les tests ont donc servi de garde-fou même sans pur RED-then-GREEN.

23 tests STEP 1 → +7 STEP 2 → 30 au total dans `commands::export`.

### STEP 2 — commande Tauri (`file_export`)

Signature révisée par rapport au plan initial : `file_export(content: String, target_path: String)` au lieu de `(vault_id, relative_path, target_path)`.

Raison : laisse le front décider quoi exporter (buffer BlockNote courant non-saved OU fichier lu depuis disk via `file_read`). Plus simple, plus testable, pas besoin d'`AppHandle`.

Validations :
- target_path absolu (le file picker système le garantit)
- pas de caractères de contrôle (anti NUL byte)
- parent du target doit exister (échec franc plutôt que `create_dir_all` à l'aveugle)

Overwrite implicite (confiance au dialog macOS qui a déjà confirmé avec l'utilisateur).

### STEP 3 — API front + helper file picker

Ajout de `fileExport(content, targetPath)` dans `src/lib/tauri/api.ts`.

Nouveau module `src/lib/utils/export.ts` :
- `defaultExportFilename(relativePath)` — basename si .md/.markdown, ajoute .md sinon, fallback `untitled.md`
- `pickAndExportMarkdown(content, defaultFilename)` — ouvre `@tauri-apps/plugin-dialog.save()` puis appelle fileExport, retourne `{ canceled } | { canceled: false, path }`
- `runExportWithToast(content, defaultFilename)` — wrapper qui ajoute le toast (success / error), cancel reste silencieux. Centralisé pour que les 3 entrées UI aient exactement le même comportement.

7 tests Vitest avec mocks (defaultExportFilename ×4, pickAndExportMarkdown ×3 : cancel / happy path / propagation d'erreur).

### STEP 4 — 3 entrées UI

1. **Command palette** (`catalog.ts`) — commande `file.export`, label "Export File…", groupe File, `when: activeFile !== null`. Exporte `activeFileStore.content` (buffer courant du tab actif).
2. **Bouton status bar** (`StatusBar.svelte`) — icône `Download` lucide, prop `onExport`. Visible si `relativePath`. Placé entre le pill de save status et le bouton thème, à droite du content-width slider. `+page.svelte` câble vers `exportActiveFile()` qui réutilise le helper.
3. **Sidebar context menu** (`Sidebar.svelte`) — item "Exporter…" sur clic-droit fichier, après "Révéler dans le Finder". Lit depuis disk via `fileRead` puis `runExportWithToast`. **Décision** : sidebar ne lit pas le buffer courant car le fichier peut ne pas être ouvert ; et si ouvert, l'autosave debounce assure qu'en pratique disk ≈ buffer.

## Smoke test

Matheo a fait tourner l'app en réel après le GATE 4. Retour : « ça marche super, frontmatter propre, clean ». Les 5 scénarios du GATE (palette, bouton, context menu, frontmatter messy → propre, cancel silencieux) sont validés.

## Tests finaux

- cargo : **156/156 ✅** (126 → 156, +30 sur `commands::export`)
- vitest : **512/512 ✅** (505 → 512, +7 sur `utils/export`)
- svelte-check : **0 / 0 ✅**
- Aucun test désactivé.

## Décisions hors plan

- **Helper `runExportWithToast` centralisé** plutôt que toast inline dans chaque caller (3 sites). Couplage `utils/export.ts` → `stores/toast` accepté pour le DRY.
- **Sidebar = lit depuis disk** (au lieu du buffer courant) — cohérent avec l'affordance "exporter ce fichier (qu'il soit ouvert ou non)".
- **Pas de raccourci clavier dédié** (`Cmd+Shift+E` proposé puis écarté à l'aiguillage). Cmd+K → "Export" reste rapide.
- **Pas de PLAN-EXPORT-MD.md formel** créé — chantier 1-session, géré inline dans la conversation, plan annoncé en 4 STEPS au début. Si on veut le formaliser a posteriori c'est possible, sinon ça vit dans cette entrée JOURNAL + le tableau de STATE.md.
- **Push autorisé explicitement** cette fois par Matheo (« commit, pousse, met à jour les journaux »). Override ponctuel de la règle "push origin reste à Matheo".

## Working tree au moment de cette entrée

- `feat/export-md` branche locale créée puis fast-forward-mergée sur main, à supprimer après push.
- `feat/editor-polish` toujours là (commit WIP `cb9dbcf`), inchangée.
- `feat/ui-port-pencil` toujours là (3 commits docs/observer + scaffold .pen), inchangée.
- `Pencil/markhub.pen` toujours pas committé sur main (il est sur `feat/ui-port-pencil` via 81f7a28).

## Frustrations

Rien à signaler. Petit chantier, scope tenu, smoke test validé du premier coup, pas de bug post-livraison.
